...
import conparator from "@/utils/conparator"
// HOOKS
...
export function StockDrawer({id, storageUnit, stock}: Props ) {
  const { updateStockQuery } = useProduct()  
  ...
  const changeProductStock = async () => {
    const response = await updateStockQuery.mutateAsync({id, stock: inputStock})
  ...
  return (
    ...
    >
      <DrawerTrigger asChild>
        <Button 
          variant= {storageUnit.unit === 1 && stock <= 10 ? 'destructive' : 'outline'}  
        >
