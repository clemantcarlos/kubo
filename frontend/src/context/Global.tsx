...
import { deleteQuery, getQuery, postQueryFormData, putQueryFormData, putQueryJson } from "@/lib/api/queries";
...
function useGlobalReducer (){
  ...
  const getProduct = useCallback(async (id:number) => {
    ...
    const findProduct = productState.data.find(product => product.id === id)

    if (findProduct) return { success: true, data: findProduct, meta: productState.meta }
  ...
  const updateStock = useCallback(async (id:number, stock:number) => {
    const controller = new AbortController();
    try {
      const changedStockProduct = await putQueryJson<Product, {stock: number}>(
