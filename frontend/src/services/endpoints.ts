...
export const API_ENDPOINTS = {
  ...
  SUPPLIERS: {
    ...
    BASE_GET_ALL: `${API_INVENTORY_URL}/supplier?all=true`,
    BASE_GET: (page: number, limit: number, search?: string, all?:boolean) => {
      if (all === true) return `${API_INVENTORY_URL}/supplier?all=true`;
      return `${API_INVENTORY_URL}/supplier?page=${page}&limit=${limit}&search=${search}`;
    },
